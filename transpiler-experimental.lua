-- transpiler.lua v16
-- Lua -> C AOT transpiler. Usage: lua transpiler.lua input.lua => output.c
-- Apache 2.0 licence
-- v16: renamed stdlib globals to avoid C conflicts (math->__lua_math etc),
--      general size reduction, kept all v15 fixes.
        ...
        for _,op in ipairs(ops) do
            ...
            if i+ol-1 <= len and expr:sub(i,i+ol-1) == op then
                if op:match("%a") then
                    ...
                    local bef = expr:sub(i+ls-2, i+ls-2)
                    local aft = expr:sub(i+le,   i+le)
...
-- Stdlib global name mapping (avoid C conflicts like 'string', 'math' near headers)
local stdlib_cname = {math="__lua_math", os="__lua_os", io="__lua_io",
                      string="__lua_string", arg="__lua_arg"}

local function global_cname(name)
    return stdlib_cname[name] or name
end
...
local function translate_expr(expr)
    ...
    if expr:match("^function%s*%(") then
        return '(__extension__({lua_runtime_error("anonymous functions not supported in AOT");LUA_UNREACHABLE();make_nil();}))'
    end
...
"..translate_expr(trim(expr:sub(li+1)))..")"
    ...
    -- string.fn(...) calls -> use C name for global
    ...
    do
        ...
        if ss and st=='call' then
            ...
            for ai,a in ipairs(args) do
                if trim(a)=="..." and ai==#args then
                    ...
                    if dobj then
                        local tbl_c = global_cname(dobj)
                        return ("table_call_splice(value_copy(%s),make_string(\"%s\"),%s)"):format(
                            tbl_c, escape_c(dkey), sep)
                    end
                    local fn_val = fn_names[fn_e] and ("make_func(%s)"):format(fn_e) or ("value_copy(%s)"):format(global_cname(fn_e))
            ...
            if dobj then
                local tbl_c = global_cname(dobj)
                return ("table_call(value_copy(%s),make_string(\"%s\"),%d%s)"):format(
                    tbl_c, escape_c(dkey), #parts, sep)
            end
            local fn_val = fn_names[fn_e] and ("make_func(%s)"):format(fn_e) or ("value_copy(%s)"):format(global_cname(fn_e))
        ...
        elseif ss and st=='dot' then
            ...
            if key and te ~= "" then
                local tbl_c = global_cname(te)
                return ('table_get(value_copy(%s),make_string("%s"))'):format(tbl_c, escape_c(key))
            end
        end
    end
    ...
    if is_id(expr) then
        ...
        return "value_copy("..global_cname(expr)..")"
    end
...
local function emit_scope_cleanup(ctx)
    ...
    if ctx.local_vars then
        for v in pairs(ctx.local_vars) do code = code..("    free_value(%s);\n"):format(v) end
    end
    return code
end
...
local function gen_ipairs_loop(tbl_expr, vars, for_body, ctx)
    ...
    ctx.break_cleanup = function()
        local bc = ""
        if ctx.local_vars then
            for v in pairs(ctx.local_vars) do bc = bc..("        free_value(%s);\n"):format(v) end
        end
        return bc
    end
...
local function gen_for(s, ctx, idx, src)
    ...
    if var then
        ...
        local prev_bflag2 = ctx.break_flag; local prev_bclean2 = ctx.break_cleanup
        local bflag2 = newtmp()
        code = code..("            int %s=0;\n"):format(bflag2)
        ...
        ctx.break_flag = prev_bflag2; ctx.break_cleanup = prev_bclean2
        ...
        local saved_lv,saved_am,before = loop_save(ctx)
        local prev_bflag = ctx.break_flag; local prev_bclean = ctx.break_cleanup
        local bflag = newtmp()
        code = code..("            int %s=0;\n"):format(bflag)
        ctx.break_flag = bflag
        ctx.break_cleanup = function()
        ...
        local bi = 1
        while bi <= #for_body do local st,c2=gen_stmt(for_body,bi,ctx); code=code..st; bi=bi+c2 end
        ctx.break_flag = prev_bflag; ctx.break_cleanup = prev_bclean
    ...
    if vars_str and iter_expr then
        ...
        if pairs_inner then
            ...
            ctx.break_cleanup = function()
                local bc = ""
                if ctx.local_vars then
                    for v,_ in pairs(ctx.local_vars) do bc = bc..("        free_value(%s);\n"):format(v) end
                end
                return bc
            end
...
local function try_table_assign(s)
    ...
    if tbl then
        local tbl_c = global_cname(tbl)
        return ("    table_set(value_copy(%s),make_string(\"%s\"),%s);\n"):format(tbl_c,escape_c(key),translate_expr(rhs))
    end
    local t2,k2,r2 = s:match("^([A-Za-z_][A-Za-z0-9_]*)%[(.+)%]%s*=%s*(.+)$")
    if t2 then
        local t2_c = global_cname(t2)
        return ("    table_set(value_copy(%s),%s,%s);\n"):format(t2_c,translate_expr(k2),translate_expr(r2))
    end
end
...
gen_stmt = function(src, idx, ctx)
    ...
    if var and rhs2 then
        am[var] = true
        local cvar = global_cname(var)
        if lv and not lv[var] and not gt[var] and not builtin_globals[var] then
            lv[var]=true
            return ("    Value %s=%s;\n"):format(var,translate_expr(rhs2)),1
        end
        local tmp = newtmp()
        return ("    Value %s=%s;\n    free_value(%s);\n    %s=%s;\n"):format(tmp,translate_expr(rhs2),cvar,cvar,tmp),1
    end
...
w([[typedef enum{T_INT,T_DOUBLE,T_STRING,T_BOOL,T_NIL,T_TABLE,T_FUNC,T_FILE}Type;
...
w([[static void free_value(Value v);
static Value value_copy(Value v);
static int value_is_truthy(Value v);
static void lua_runtime_error(const char*fmt,...){va_list ap;va_start(ap,fmt);fprintf(stderr,"runtime error: ");vfprintf(stderr,fmt,ap);fprintf(stderr,"\n");va_end(ap);abort();}
...
static inline void retslot_save(void){if(__retslot_top>=RETSLOT_MAX)lua_runtime_error("call stack overflow");RetSlot*s=&__retslots[__retslot_top++];s->cnt=__retncount;for(int _i=1;_i<__retncount&&_i<RETBUF_CAP;_i++)s->sec[_i]=__retbuf[_i];__retncount=0;}
static inline void retslot_restore(void){if(__retslot_top<=0)return;int callee_cnt=__retncount;Value callee_sec[RETBUF_CAP];for(int _i=1;_i<callee_cnt&&_i<RETBUF_CAP;_i++)callee_sec[_i]=__retbuf[_i];RetSlot*s=&__retslots[--__retslot_top];__retncount=s->cnt;for(int _i=1;_i<s->cnt&&_i<RETBUF_CAP;_i++)__retbuf[_i]=s->sec[_i];__retncount=callee_cnt;for(int _i=1;_i<callee_cnt&&_i<RETBUF_CAP;_i++)__retbuf[_i]=callee_sec[_i];}]])

if has_tables then w([[typedef struct TableEntry{Value key;Value val;int used;}TableEntry;
...
if has_tables then w([[static Table*table_new(void){Table*t=calloc(1,sizeof(Table));t->cap=TBL_INIT_CAP;t->entries=calloc(t->cap,sizeof(TableEntry));t->refcount=1;t->array_len=0;return t;}
...
static void table_resize(Table*t){int oc=t->cap;TableEntry*old=t->entries;t->cap*=2;t->entries=calloc(t->cap,sizeof(TableEntry));t->len=0;t->array_len=0;for(int i=0;i<oc;i++){if(!old[i].used)continue;table_raw_set(t,old[i].key,old[i].val);old[i].used=0;}free(old);}
static void table_raw_delete(Table*t,Value key){uint32_t h=hash_value(key);int mask=t->cap-1,idx=(int)(h&mask),chk=0;while(t->entries[idx].used&&chk<t->cap){if(keys_equal(t->entries[idx].key,key)){if(key.t==T_INT&&key.i==(int64_t)t->array_len){t->array_len--;while(t->array_len>0){int64_t ck=(int64_t)t->array_len;uint32_t ch=(uint32_t)(ck*2654435761ULL);int ci=(int)(ch&mask),cc=0;int found=0;while(t->entries[ci].used&&cc<t->cap){if(t->entries[ci].key.t==T_INT&&t->entries[ci].key.i==ck){found=1;break;}ci=(ci+1)&mask;cc++;}if(found)break;t->array_len--;}}free_value(t->entries[idx].key);free_value(t->entries[idx].val);t->entries[idx].used=0;t->len--;int prev2=idx;idx=(idx+1)&mask;while(t->entries[idx].used){int nat=(int)(hash_value(t->entries[idx].key)&mask);int should_move=(idx>prev2)?(nat<=prev2||nat>idx):(nat<=prev2&&nat>idx);if(should_move){t->entries[prev2]=t->entries[idx];t->entries[idx].used=0;prev2=idx;}idx=(idx+1)&mask;}free_value(key);return;}idx=(idx+1)&mask;chk++;}free_value(key);}
static void table_raw_set(Table*t,Value key,Value val){if(val.t==T_NIL){free_value(val);table_raw_delete(t,key);return;}if(key.t==T_DOUBLE&&key.d!=key.d){free_value(key);free_value(val);lua_runtime_error("table index is NaN");}if(t->len*2>=t->cap)table_resize(t);uint32_t h=hash_value(key);int mask=t->cap-1,idx=(int)(h&mask);while(t->entries[idx].used){if(keys_equal(t->entries[idx].key,key)){free_value(t->entries[idx].key);free_value(t->entries[idx].val);t->entries[idx].key=key;t->entries[idx].val=val;return;}idx=(idx+1)&mask;}t->entries[idx].used=1;t->entries[idx].key=key;t->entries[idx].val=val;t->len++;int64_t ki=-1;if(key.t==T_INT)ki=key.i;else if(key.t==T_DOUBLE){double d=key.d;if(d>=(double)1&&d<=(double)INT64_MAX&&floor(d)==d)ki=(int64_t)d;}if(ki==(int64_t)(t->array_len+1)){t->array_len++;while(1){int64_t nk=(int64_t)(t->array_len+1);uint32_t nh=(uint32_t)(nk*2654435761ULL);int ni=(int)(nh&mask),chk=0;while(t->entries[ni].used&&chk<t->cap){if(t->entries[ni].key.t==T_INT&&t->entries[ni].key.i==nk){t->array_len++;goto next_key;}if(t->entries[ni].key.t==T_DOUBLE){double d2=t->entries[ni].key.d;if(floor(d2)==d2&&(int64_t)d2==nk){t->array_len++;goto next_key;}}ni=(ni+1)&mask;chk++;}break;next_key:;}}}
...
static Value table_get(Value tbl,Value key){if(tbl.t!=T_TABLE||!tbl.tbl){lua_runtime_error("attempt to index a %s value",(tbl.t==T_NIL?"nil":tbl.t==T_BOOL?"boolean":tbl.t==T_INT||tbl.t==T_DOUBLE?"number":tbl.t==T_FUNC?"function":"?"));free_value(tbl);free_value(key);return(Value){.t=T_NIL};}if(key.t==T_DOUBLE&&key.d!=key.d){free_value(tbl);free_value(key);return(Value){.t=T_NIL};}Table*t=tbl.tbl;uint32_t h=hash_value(key);int mask=t->cap-1,idx=(int)(h&mask),chk=0;while(t->entries[idx].used&&chk<t->cap){if(keys_equal(t->entries[idx].key,key)){Value r=value_copy(t->entries[idx].val);free_value(tbl);free_value(key);return r;}idx=(idx+1)&mask;chk++;}free_value(tbl);free_value(key);return(Value){.t=T_NIL};}
static Value table_call(Value tbl,Value key,int nargs,...);]]) end

w([[static inline Value make_int(int64_t x){Value v;v.t=T_INT;v.i=x;return v;}
...
wif(use.table_init,[[static Value table_init(int n,...){Value tbl=make_table();va_list ap;va_start(ap,n);for(int i=0;i<n;i++){Value k=va_arg(ap,Value),v=va_arg(ap,Value);table_raw_set(tbl.tbl,k,v);}va_end(ap);return tbl;}]])
w([[static void free_value(Value v){if(v.t==T_STRING)luastr_decref(v.ls);]])
...
wif(use.use_select or use.use_vararg,[[static Value lua_select_vararg(Value idx,int fixed,int nargs,Value*args){if(idx.t==T_STRING&&idx.ls&&idx.ls->data[0]=='#'&&idx.ls->data[1]=='\0'){free_value(idx);return make_int(nargs-fixed);}int64_t n=(idx.t==T_INT)?idx.i:(int64_t)floor(idx.d);free_value(idx);if(n<0)n=(nargs-fixed)+n+1;int pos=fixed+(int)n-1;return(pos>=0&&pos<nargs)?value_copy(args[pos]):make_nil();}]])

w([[static Value value_call(Value fn,int nargs,...){Value stack_args[16];Value*args=NULL;if(nargs>0){if(nargs<=16)args=stack_args;else{args=malloc(nargs*sizeof(Value));if(!args){free_value(fn);lua_runtime_error("out of memory");LUA_UNREACHABLE();}}}va_list ap;va_start(ap,nargs);for(int i=0;i<nargs;i++)args[i]=va_arg(ap,Value);va_end(ap);if(fn.t!=T_FUNC||!fn.fn){for(int i=0;i<nargs;i++)free_value(args[i]);if(nargs>16)free(args);const char*tn=fn.t==T_NIL?"nil":fn.t==T_BOOL?"boolean":fn.t==T_INT||fn.t==T_DOUBLE?"number":fn.t==T_STRING?"string":fn.t==T_TABLE?"table":"?";free_value(fn);lua_runtime_error("attempt to call a %s value",tn);LUA_UNREACHABLE();return make_nil();}retslot_save();Value r=fn.fn(nargs,args);retslot_restore();free_value(fn);if(nargs>16)free(args);return r;}
static Value value_call_splice(Value fn,int extra_count,Value*extra_args){if(fn.t!=T_FUNC||!fn.fn){for(int i=0;i<extra_count;i++)free_value(extra_args[i]);const char*tn=fn.t==T_NIL?"nil":fn.t==T_BOOL?"boolean":fn.t==T_INT||fn.t==T_DOUBLE?"number":fn.t==T_STRING?"string":fn.t==T_TABLE?"table":"?";free_value(fn);lua_runtime_error("attempt to call a %s value",tn);LUA_UNREACHABLE();return make_nil();}retslot_save();Value r=fn.fn(extra_count,extra_args);retslot_restore();free_value(fn);return r;}]])

wif(has_tables,[[static Value table_call(Value tbl,Value key,int nargs,...){Value fn=table_get(value_copy(tbl),value_copy(key));free_value(tbl);free_value(key);Value stack_args[16];Value*args=NULL;if(nargs>0){if(nargs<=16)args=stack_args;else{args=malloc(nargs*sizeof(Value));if(!args){free_value(fn);lua_runtime_error("out of memory");LUA_UNREACHABLE();}}}va_list ap;va_start(ap,nargs);for(int i=0;i<nargs;i++)args[i]=va_arg(ap,Value);va_end(ap);if(fn.t!=T_FUNC||!fn.fn){for(int i=0;i<nargs;i++)free_value(args[i]);if(nargs>16)free(args);const char*tn=fn.t==T_NIL?"nil":fn.t==T_BOOL?"boolean":fn.t==T_INT||fn.t==T_DOUBLE?"number":fn.t==T_STRING?"string":fn.t==T_TABLE?"table":"?";free_value(fn);lua_runtime_error("attempt to call a %s value",tn);LUA_UNREACHABLE();return make_nil();}retslot_save();Value r=fn.fn(nargs,args);retslot_restore();free_value(fn);if(nargs>16)free(args);return r;}
static Value table_call_splice(Value tbl,Value key,int extra_count,Value*extra_args){Value fn=table_get(value_copy(tbl),value_copy(key));free_value(tbl);free_value(key);if(fn.t!=T_FUNC||!fn.fn){for(int i=0;i<extra_count;i++)free_value(extra_args[i]);const char*tn=fn.t==T_NIL?"nil":fn.t==T_BOOL?"boolean":fn.t==T_INT||fn.t==T_DOUBLE?"number":fn.t==T_STRING?"string":fn.t==T_TABLE?"table":"?";free_value(fn);lua_runtime_error("attempt to call a %s value",tn);LUA_UNREACHABLE();return make_nil();}retslot_save();Value r=fn.fn(extra_count,extra_args);retslot_restore();free_value(fn);return r;}]])

wif(use.method_call,[[static Value value_method_call(Value self,const char*method,int nargs,...){Value fn=table_get(value_copy(self),make_string(method));int total=nargs+1;Value stack_args[17];Value*args=total<=17?stack_args:malloc(total*sizeof(Value));if(!args){free_value(self);free_value(fn);lua_runtime_error("out of memory");LUA_UNREACHABLE();}args[0]=self;va_list ap;va_start(ap,nargs);for(int i=0;i<nargs;i++)args[i+1]=va_arg(ap,Value);va_end(ap);if(fn.t!=T_FUNC||!fn.fn){for(int i=0;i<total;i++)free_value(args[i]);if(total>17)free(args);free_value(fn);lua_runtime_error("attempt to call method '%s' (a %s value)",method,fn.t==T_NIL?"nil":fn.t==T_TABLE?"table":"?");LUA_UNREACHABLE();return make_nil();}retslot_save();Value r=fn.fn(total,args);retslot_restore();free_value(fn);if(total>17)free(args);return r;}]])

wif(use.use_tostring or use.print,[[static Value lua_tostring(Value v){char buf[64];switch(v.t){case T_STRING:return v;case T_INT:snprintf(buf,sizeof buf,"%lld",(long long)v.i);free_value(v);return make_string(buf);case T_DOUBLE:snprintf(buf,sizeof buf,"%g",v.d);free_value(v);return make_string(buf);case T_BOOL:{const char*s=v.i?"true":"false";free_value(v);return make_string(s);}case T_NIL:free_value(v);return make_string("nil");case T_TABLE:snprintf(buf,sizeof buf,"table:%p",(void*)v.tbl);free_value(v);return make_string(buf);case T_FUNC:snprintf(buf,sizeof buf,"function:%p",(void*)(size_t)v.fn);free_value(v);return make_string(buf);default:free_value(v);return make_string("?");}}]])
wif(use.use_tonumber,[[static Value lua_tonumber(Value v){if(v.t==T_INT||v.t==T_DOUBLE)return v;if(v.t==T_STRING&&v.ls){char*end;const char*s=v.ls->data;long long iv=strtoll(s,&end,0);if(*end=='\0'){free_value(v);return make_int(iv);}double dv=strtod(s,&end);if(*end=='\0'){free_value(v);return make_double(dv);}}free_value(v);return make_nil();}]])
wif(use.use_type,[[static Value lua_type(Value v){const char*s;switch(v.t){case T_INT:case T_DOUBLE:s="number";break;case T_STRING:s="string";break;case T_BOOL:s="boolean";break;case T_NIL:s="nil";break;case T_TABLE:s="table";break;case T_FUNC:s="function";break;case T_FILE:s="file";break;default:s="userdata";}free_value(v);return make_string(s);}]])
wif(use.error or use.assert,[[static Value lua_error(Value msg){const char*m=(msg.t==T_STRING)?luastr_cstr(msg.ls):"(error object)";fprintf(stderr,"error: %s\n",m);free_value(msg);abort();return make_nil();}]])
wif(use.assert,[[static Value lua_assert(Value cond,Value msg){if(!value_is_truthy(cond)){const char*m=(msg.t==T_STRING)?luastr_cstr(msg.ls):"assertion failed!";fprintf(stderr,"assertion failed: %s\n",m);free_value(cond);free_value(msg);abort();}free_value(msg);return cond;}]])
wif(use.print,[[static void print_values(int n,Value*args){size_t cap=256;char*buf=malloc(cap);if(!buf)return;size_t len=0;for(int i=0;i<n;i++){if(i>0){if(len+1>=cap){cap*=2;void*__rb=realloc(buf,cap);if(!__rb){free(buf);return;}buf=(char*)__rb;}buf[len++]='\t';}Value v=args[i];char tmp[64];const char*s=NULL;size_t slen=0;switch(v.t){case T_INT:slen=(size_t)snprintf(tmp,sizeof tmp,"%lld",(long long)v.i);s=tmp;break;case T_DOUBLE:slen=(size_t)snprintf(tmp,sizeof tmp,"%g",v.d);s=tmp;break;case T_STRING:s=luastr_cstr(v.ls);slen=strlen(s);break;case T_BOOL:s=v.i?"true":"false";slen=v.i?4u:5u;break;case T_NIL:s="nil";slen=3;break;case T_TABLE:slen=(size_t)snprintf(tmp,sizeof tmp,"table:%p",(void*)v.tbl);s=tmp;break;case T_FUNC:slen=(size_t)snprintf(tmp,sizeof tmp,"function:%p",(void*)(size_t)v.fn);s=tmp;break;case T_FILE:slen=(size_t)snprintf(tmp,sizeof tmp,"file:%p",(void*)v.file);s=tmp;break;default:s="";slen=0;break;}if(len+slen+1>=cap){while(len+slen+1>=cap)cap*=2;void*__rb=realloc(buf,cap);if(!__rb){free(buf);return;}buf=(char*)__rb;}if(slen>0)memcpy(buf+len,s,slen);len+=slen;}if(len+1>=cap){cap=len+1;void*__rb=realloc(buf,cap);if(!__rb){free(buf);return;}buf=(char*)__rb;}buf[len++]='\n';fwrite(buf,1,len,stdout);free(buf);for(int i=0;i<n;i++)free_value(args[i]);}]])

w([[#define LUA_NUM_CHECK2(a,b) do{if((a).t!=T_INT&&(a).t!=T_DOUBLE){lua_runtime_error("attempt to perform arithmetic on a %s value",(a).t==T_NIL?"nil":(a).t==T_STRING?"string":(a).t==T_BOOL?"boolean":"?");free_value(a);free_value(b);return make_nil();}if((b).t!=T_INT&&(b).t!=T_DOUBLE){lua_runtime_error("attempt to perform arithmetic on a %s value",(b).t==T_NIL?"nil":(b).t==T_STRING?"string":(b).t==T_BOOL?"boolean":"?");free_value(a);free_value(b);return make_nil();}}while(0)
...
wif(use.value_len,[[static Value value_len(Value v){if(v.t==T_STRING){int64_t l=v.ls?(int64_t)strlen(v.ls->data):0;free_value(v);return make_int(l);}if(v.t==T_TABLE&&v.tbl){int64_t l=(int64_t)v.tbl->array_len;free_value(v);return make_int(l);}lua_runtime_error("attempt to get length of a %s value",v.t==T_NIL?"nil":v.t==T_BOOL?"boolean":v.t==T_INT||v.t==T_DOUBLE?"number":v.t==T_FUNC?"function":"?");free_value(v);return make_int(0);}]])
wif(use.value_concat,[[static Value value_concat(Value a,Value b){char abuf[64],bbuf[64];const char*as=NULL;const char*bs=NULL;if(a.t==T_STRING)as=luastr_cstr(a.ls);else if(a.t==T_INT){snprintf(abuf,sizeof abuf,"%lld",(long long)a.i);as=abuf;}else if(a.t==T_DOUBLE){snprintf(abuf,sizeof abuf,"%g",a.d);as=abuf;}else{lua_runtime_error("attempt to concatenate a %s value",a.t==T_NIL?"nil":a.t==T_BOOL?"boolean":a.t==T_TABLE?"table":a.t==T_FUNC?"function":"?");free_value(a);free_value(b);return make_string("");}if(b.t==T_STRING)bs=luastr_cstr(b.ls);else if(b.t==T_INT){snprintf(bbuf,sizeof bbuf,"%lld",(long long)b.i);bs=bbuf;}else if(b.t==T_DOUBLE){snprintf(bbuf,sizeof bbuf,"%g",b.d);bs=bbuf;}else{lua_runtime_error("attempt to concatenate a %s value",b.t==T_NIL?"nil":b.t==T_BOOL?"boolean":b.t==T_TABLE?"table":b.t==T_FUNC?"function":"?");free_value(a);free_value(b);return make_string("");}size_t la=as?strlen(as):0,lb=bs?strlen(bs):0;LuaStr*ls=luastr_new(NULL,la+lb);if(la>0&&as)memcpy(ls->data,as,la);if(lb>0&&bs)memcpy(ls->data+la,bs,lb);free_value(a);free_value(b);Value r;r.t=T_STRING;r.ls=ls;return r;}]])
wif(use.value_add,[[static Value value_add(Value a,Value b){LUA_NUM_CHECK2(a,b);Value r=(a.t==T_INT&&b.t==T_INT)?make_int(I64_ADD(a.i,b.i)):make_double(numval(a)+numval(b));free_value(a);free_value(b);return r;}]])
wif(use.value_sub,[[static Value value_sub(Value a,Value b){LUA_NUM_CHECK2(a,b);Value r=(a.t==T_INT&&b.t==T_INT)?make_int(I64_SUB(a.i,b.i)):make_double(numval(a)-numval(b));free_value(a);free_value(b);return r;}]])
wif(use.value_mul,[[static Value value_mul(Value a,Value b){LUA_NUM_CHECK2(a,b);Value r=(a.t==T_INT&&b.t==T_INT)?make_int(I64_MUL(a.i,b.i)):make_double(numval(a)*numval(b));free_value(a);free_value(b);return r;}]])
wif(use.value_div,[[static Value value_div(Value a,Value b){LUA_NUM_CHECK2(a,b);if(a.t==T_INT&&b.t==T_INT){if(b.i==0){free_value(a);free_value(b);lua_runtime_error("attempt to divide by zero");}double r=(double)a.i/(double)b.i;free_value(a);free_value(b);return make_double(r);}double r=numval(a)/numval(b);free_value(a);free_value(b);return make_double(r);}]])
wif(use.value_mod,[[static Value value_mod(Value a,Value b){LUA_NUM_CHECK2(a,b);if(a.t==T_INT&&b.t==T_INT){if(b.i==0){free_value(a);free_value(b);lua_runtime_error("attempt to perform 'n%%0'");}int64_t r=((a.i%b.i)+b.i)%b.i;free_value(a);free_value(b);return make_int(r);}double da=numval(a),db=numval(b);free_value(a);free_value(b);if(db==0.0)return make_double(0.0/0.0);return make_double(da-floor(da/db)*db);}]])
wif(use.value_pow,[[static Value value_pow(Value a,Value b){LUA_NUM_CHECK2(a,b);double r=pow(numval(a),numval(b));free_value(a);free_value(b);return make_double(r);}]])
...
w([[#define LUA_TYPENAME(v) ((v).t==T_NIL?"nil":(v).t==T_BOOL?"boolean":(v).t==T_INT||(v).t==T_DOUBLE?"number":(v).t==T_STRING?"string":(v).t==T_TABLE?"table":(v).t==T_FUNC?"function":"?")]])
wif(use.value_lt,[[static Value value_lt(Value a,Value b){if((a.t==T_INT||a.t==T_DOUBLE)&&(b.t==T_INT||b.t==T_DOUBLE)){int r=(numval(a)<numval(b));free_value(a);free_value(b);return make_bool(r);}if(a.t==T_STRING&&b.t==T_STRING){int r=(strcmp(luastr_cstr(a.ls),luastr_cstr(b.ls))<0);free_value(a);free_value(b);return make_bool(r);}lua_runtime_error("attempt to compare %s with %s",LUA_TYPENAME(a),LUA_TYPENAME(b));free_value(a);free_value(b);return make_bool(0);}]])
wif(use.value_gt,"static Value value_gt(Value a,Value b){return value_lt(b,a);}")
wif(use.value_le,[[static Value value_le(Value a,Value b){if((a.t==T_INT||a.t==T_DOUBLE)&&(b.t==T_INT||b.t==T_DOUBLE)){int r=(numval(a)<=numval(b));free_value(a);free_value(b);return make_bool(r);}if(a.t==T_STRING&&b.t==T_STRING){int r=(strcmp(luastr_cstr(a.ls),luastr_cstr(b.ls))<=0);free_value(a);free_value(b);return make_bool(r);}lua_runtime_error("attempt to compare %s with %s",LUA_TYPENAME(a),LUA_TYPENAME(b));free_value(a);free_value(b);return make_bool(0);}]])
wif(use.value_ge,[[static Value value_ge(Value a,Value b){if((a.t==T_INT||a.t==T_DOUBLE)&&(b.t==T_INT||b.t==T_DOUBLE)){int r=(numval(a)>=numval(b));free_value(a);free_value(b);return make_bool(r);}if(a.t==T_STRING&&b.t==T_STRING){int r=(strcmp(luastr_cstr(a.ls),luastr_cstr(b.ls))>=0);free_value(a);free_value(b);return make_bool(r);}lua_runtime_error("attempt to compare %s with %s",LUA_TYPENAME(a),LUA_TYPENAME(b));free_value(a);free_value(b);return make_bool(0);}]])
...
w([==[
typedef struct{const char*src_init,*src_end,*p_end;struct{const char*init;ptrdiff_t len;}capture[32];int level;}MatchState;
...
static const char*match(MatchState*ms,const char*s,const char*p){if(p>=ms->p_end)return s;switch(*p){case'(':{const char*res;int l=ms->level;if(*(p+1)==')'){ms->capture[l].init=s;ms->capture[l].len=CAP_POSITION;ms->level=l+1;res=match(ms,s,p+2);if(!res)ms->level=l;}else{ms->capture[l].init=s;ms->capture[l].len=CAP_UNFINISHED;ms->level=l+1;res=match(ms,s,p+1);if(!res)ms->level=l;}return res;}case')':{int l=ms->level-1;while(l>=0&&ms->capture[l].len!=CAP_UNFINISHED)l--;if(l<0)return NULL;ms->capture[l].len=(ptrdiff_t)(s-ms->capture[l].init);const char*res=match(ms,s,p+1);if(!res)ms->capture[l].len=CAP_UNFINISHED;return res;}case'$':if(*(p+1)=='\0')return(s==ms->src_end?s:NULL);default:{const char*ep=classend(p);int m;if(*p=='[')m=matchbracketclass((unsigned char)*s,p,ep-1)&&s<ms->src_end;else m=singlematch(ms,s,p);switch(*ep){case'?':{const char*res;if(m&&(res=match(ms,s+1,ep+1)))return res;return match(ms,s,ep+1);}case'+':{return m?max_expand(ms,s+1,p,ep+1):NULL;}case'*':{return max_expand(ms,s,p,ep+1);}case'-':{return min_expand(ms,s,p,ep+1);}default:{if(!m)return NULL;s++;p=ep;return match(ms,s,p);}}}case'%':{switch(*(p+1)){case'b':{s=matchbalance(ms,s,p+2);if(!s)return NULL;p+=4;return match(ms,s,p);}default:{const char*ep=classend(p);int m;if(*p=='[')m=matchbracketclass((unsigned char)*s,p,ep-1)&&s<ms->src_end;else m=singlematch(ms,s,p);switch(*ep){case'?':{const char*res;if(m&&(res=match(ms,s+1,ep+1)))return res;return match(ms,s,ep+1);}case'+':{return m?max_expand(ms,s+1,p,ep+1):NULL;}case'*':{return max_expand(ms,s,p,ep+1);}case'-':{return min_expand(ms,s,p,ep+1);}default:{if(!m)return NULL;return match(ms,s+1,ep);}}}}}}}
static int push_captures(MatchState*ms,const char*s,const char*e,int off){int nlevels=(ms->level==0&&s)?1:ms->level,count=0;for(int i=0;i<nlevels;i++){if(i+off>=__retbuf_cap)break;if(ms->level==0)__retbuf[off+i]=make_string_n(s,(size_t)(e-s));else if(ms->capture[i].len==CAP_POSITION)__retbuf[off+i]=make_int(ms->capture[i].init-ms->src_init+1);else __retbuf[off+i]=make_string_n(ms->capture[i].init,(size_t)ms->capture[i].len);count++;}return count;}
...
static Value lua_string_sub(int n,Value*a){if(n<1||a[0].t!=T_STRING)STR_FREE_RET(n,a,make_string(""));const char*s=luastr_cstr(a[0].ls);int64_t sl=(int64_t)strlen(s);int64_t i=n>1?(int64_t)numval(a[1]):1,j=n>2?(int64_t)numval(a[2]):sl;if(i<0)i=sl+i+1;if(j<0)j=sl+j+1;if(i<1)i=1;if(j>sl)j=sl;Value r=(i>j)?make_string(""):make_string_n(s+(i-1),(size_t)(j-i+1));STR_FREE_RET(n,a,r);}
static Value lua_string_rep(int n,Value*a){if(n<2||a[0].t!=T_STRING)STR_FREE_RET(n,a,make_string(""));const char*s=luastr_cstr(a[0].ls);size_t sl=strlen(s);int64_t cnt=(int64_t)numval(a[1]);const char*sep=(n>2&&a[2].t==T_STRING)?luastr_cstr(a[2].ls):"";size_t sepl=strlen(sep);if(cnt<=0)STR_FREE_RET(n,a,make_string(""));size_t total=sl*cnt+sepl*(cnt>1?cnt-1:0);char*buf=(char*)malloc(total+1);if(!buf)STR_FREE_RET(n,a,make_string(""));size_t pos=0;for(int64_t k=0;k<cnt;k++){if(k>0&&sepl){memcpy(buf+pos,sep,sepl);pos+=sepl;}memcpy(buf+pos,s,sl);pos+=sl;}buf[total]='\0';Value r=make_string(buf);free(buf);STR_FREE_RET(n,a,r);}
static Value lua_string_reverse(int n,Value*a){if(n<1||a[0].t!=T_STRING)STR_FREE_RET(n,a,make_string(""));const char*s=luastr_cstr(a[0].ls);size_t sl=strlen(s);char*buf=malloc(sl+1);if(!buf)STR_FREE_RET(n,a,make_string(""));for(size_t k=0;k<sl;k++)buf[k]=s[sl-1-k];buf[sl]='\0';Value r=make_string(buf);free(buf);STR_FREE_RET(n,a,r);}
static Value lua_string_upper(int n,Value*a){if(n<1||a[0].t!=T_STRING)STR_FREE_RET(n,a,make_string(""));const char*s=luastr_cstr(a[0].ls);size_t sl=strlen(s);char*buf=malloc(sl+1);if(!buf)STR_FREE_RET(n,a,make_string(""));for(size_t k=0;k<sl;k++)buf[k]=(char)toupper((unsigned char)s[k]);buf[sl]='\0';Value r=make_string(buf);free(buf);STR_FREE_RET(n,a,r);}
static Value lua_string_lower(int n,Value*a){if(n<1||a[0].t!=T_STRING)STR_FREE_RET(n,a,make_string(""));const char*s=luastr_cstr(a[0].ls);size_t sl=strlen(s);char*buf=malloc(sl+1);if(!buf)STR_FREE_RET(n,a,make_string(""));for(size_t k=0;k<sl;k++)buf[k]=(char)tolower((unsigned char)s[k]);buf[sl]='\0';Value r=make_string(buf);free(buf);STR_FREE_RET(n,a,r);}
static Value lua_string_byte(int n,Value*a){if(n<1||a[0].t!=T_STRING)STR_FREE_RET(n,a,make_nil());const char*s=luastr_cstr(a[0].ls);int64_t sl=(int64_t)strlen(s);int64_t i=n>1?(int64_t)numval(a[1]):1,j=n>2?(int64_t)numval(a[2]):i;if(i<0)i=sl+i+1;if(j<0)j=sl+j+1;if(i<1)i=1;if(j>sl)j=sl;for(int k=0;k<n;k++)free_value(a[k]);if(i>j)return make_nil();int64_t cnt=j-i+1;for(int ri=1;ri<__retncount&&ri<__retbuf_cap;ri++)free_value(__retbuf[ri]);__retncount=(int)cnt;for(int64_t k=0;k<cnt;k++)if(k<__retbuf_cap)__retbuf[k]=make_int((unsigned char)s[i-1+k]);return make_int((unsigned char)s[i-1]);}
static Value lua_string_char(int n,Value*a){char*buf=malloc(n+1);if(!buf){for(int i=0;i<n;i++)free_value(a[i]);return make_string("");}for(int i=0;i<n;i++){buf[i]=(char)(int64_t)numval(a[i]);free_value(a[i]);}buf[n]='\0';Value r=make_string(buf);free(buf);return r;}
static Value lua_string_find(int n,Value*a){if(n<2||a[0].t!=T_STRING||a[1].t!=T_STRING)STR_FREE_RET(n,a,make_nil());const char*s=luastr_cstr(a[0].ls);size_t sl=strlen(s);const char*p=luastr_cstr(a[1].ls);int64_t init=n>2?(int64_t)numval(a[2]):1;int plain=n>3&&value_is_truthy(a[3]);if(init<0)init=(int64_t)sl+init+1;if(init<1)init=1;if(init>(int64_t)sl+1)init=(int64_t)sl+1;const char*s1=s+init-1;for(int i=0;i<n;i++)free_value(a[i]);if(plain){const char*found=strstr(s1,p);if(!found)return make_nil();int64_t st=(int64_t)(found-s)+1,en=st+(int64_t)strlen(p)-1;for(int ri=1;ri<__retncount&&ri<__retbuf_cap;ri++)free_value(__retbuf[ri]);__retncount=2;__retbuf[0]=make_int(st);__retbuf[1]=make_int(en);return make_int(st);}MatchState ms;ms.src_init=s;ms.src_end=s+sl;ms.p_end=p+strlen(p);int anchor=0;if(*p=='^'){anchor=1;p++;}const char*s2=s1;do{ms.level=0;const char*res=match(&ms,s2,p);if(res){int64_t st=(int64_t)(s2-s)+1,en=(int64_t)(res-s);for(int ri=1;ri<__retncount&&ri<__retbuf_cap;ri++)free_value(__retbuf[ri]);__retncount=2;__retbuf[0]=make_int(st);__retbuf[1]=make_int(en);return make_int(st);}}while(s2++<ms.src_end&&!anchor);return make_nil();}
static Value lua_string_match(int n,Value*a){if(n<2||a[0].t!=T_STRING||a[1].t!=T_STRING)STR_FREE_RET(n,a,make_nil());const char*s=luastr_cstr(a[0].ls);size_t sl=strlen(s);const char*p=luastr_cstr(a[1].ls);int64_t init=n>2?(int64_t)numval(a[2]):1;if(init<0)init=(int64_t)sl+init+1;if(init<1)init=1;if(init>(int64_t)sl+1)init=(int64_t)sl+1;const char*s1=s+init-1;for(int i=0;i<n;i++)free_value(a[i]);MatchState ms;ms.src_init=s;ms.src_end=s+sl;int anchor=0;if(*p=='^'){anchor=1;p++;}ms.p_end=p+strlen(p);const char*s2=s1;do{ms.level=0;const char*res=match(&ms,s2,p);if(res){for(int ri=0;ri<__retncount&&ri<__retbuf_cap;ri++)free_value(__retbuf[ri]);int nc=push_captures(&ms,s2,res,0);__retncount=nc;return nc>0?value_copy(__retbuf[0]):make_nil();}}while(s2++<ms.src_end&&!anchor);return make_nil();}
static Value lua_string_gmatch(int n,Value*a){if(n<2||a[0].t!=T_STRING||a[1].t!=T_STRING)STR_FREE_RET(n,a,make_nil());const char*s=luastr_cstr(a[0].ls);size_t sl=strlen(s);const char*p=luastr_cstr(a[1].ls);for(int i=0;i<n;i++)free_value(a[i]);Value tbl=make_table();int idx=1;MatchState ms;ms.src_init=s;ms.src_end=s+sl;ms.p_end=p+strlen(p);const char*src=s;while(src<=ms.src_end){ms.level=0;const char*e=match(&ms,src,p);if(e){if(ms.level>1){Value sub=make_table();for(int ci=0;ci<ms.level;ci++){Value cap;if(ms.capture[ci].len==CAP_POSITION)cap=make_int(ms.capture[ci].init-ms.src_init+1);else cap=make_string_n(ms.capture[ci].init,(size_t)ms.capture[ci].len);table_raw_set(sub.tbl,make_int(ci+1),cap);}table_raw_set(tbl.tbl,make_int(idx++),sub);}else if(ms.level==1){Value cap;if(ms.capture[0].len==CAP_POSITION)cap=make_int(ms.capture[0].init-ms.src_init+1);else cap=make_string_n(ms.capture[0].init,(size_t)ms.capture[0].len);table_raw_set(tbl.tbl,make_int(idx++),cap);}else{table_raw_set(tbl.tbl,make_int(idx++),make_string_n(src,(size_t)(e-src)));}if(e==src)src++;else src=e;}else src++;}return tbl;}
static Value lua_string_gsub(int n,Value*a){if(n<3||a[0].t!=T_STRING||a[1].t!=T_STRING)STR_FREE_RET(n,a,make_string(""));const char*s=luastr_cstr(a[0].ls);size_t sl=strlen(s);const char*p=luastr_cstr(a[1].ls);int maxn=n>3?(int)numval(a[3]):-1;size_t bufsz=sl*2+64;char*buf=malloc(bufsz);size_t buflen=0;int count=0;if(!buf)STR_FREE_RET(n,a,make_string(""));MatchState ms;ms.src_init=s;ms.src_end=s+sl;ms.p_end=p+strlen(p);int anchor=(*p=='^');if(anchor)p++;ms.p_end=p+strlen(p);const char*src=s;while((maxn<0||count<maxn)&&src<=ms.src_end){ms.level=0;const char*e=match(&ms,src,p);if(e){count++;if(a[2].t==T_STRING){const char*repl=luastr_cstr(a[2].ls);while(*repl){if(*repl!='%'){BA(*repl++);}else{repl++;if(*repl>='0'&&*repl<='9'){int ci=*repl-'0';repl++;if(ci==0){BAS(src,(size_t)(e-src));}else if(ci<=ms.level&&ms.capture[ci-1].len>=0)BAS(ms.capture[ci-1].init,(size_t)ms.capture[ci-1].len);}else{BA(*repl++);}}}}else if(a[2].t==T_TABLE){Value key=ms.level>0&&ms.capture[0].len>=0?make_string_n(ms.capture[0].init,(size_t)ms.capture[0].len):make_string_n(src,(size_t)(e-src));Value val=table_get(value_copy(a[2]),key);if(value_is_truthy(val)){Value sv=lua_tostring(value_copy(val));BAS(luastr_cstr(sv.ls),strlen(luastr_cstr(sv.ls)));free_value(sv);}else{BAS(src,(size_t)(e-src));}free_value(val);}else if(a[2].t==T_FUNC){Value fargs[32];int nf=0;if(ms.level>0){for(int ci=0;ci<ms.level&&nf<32;ci++){if(ms.capture[ci].len==CAP_POSITION)fargs[nf++]=make_int(ms.capture[ci].init-ms.src_init+1);else fargs[nf++]=make_string_n(ms.capture[ci].init,(size_t)ms.capture[ci].len);}}else{fargs[nf++]=make_string_n(src,(size_t)(e-src));}Value rv=a[2].fn(nf,fargs);if(value_is_truthy(rv)){Value sv=lua_tostring(value_copy(rv));BAS(luastr_cstr(sv.ls),strlen(luastr_cstr(sv.ls)));free_value(sv);}else{BAS(src,(size_t)(e-src));}free_value(rv);}if(e==src)BA(*src++);else src=e;}else{BA(*src++);} if(anchor)break;}if(buf){while(src<ms.src_end)BA(*src++);buf[buflen]='\0';}int64_t cnt_val=count;for(int i=0;i<n;i++)free_value(a[i]);for(int ri=1;ri<__retncount&&ri<__retbuf_cap;ri++)free_value(__retbuf[ri]);__retncount=2;Value res_str=buf?make_string(buf):make_string("");free(buf);__retbuf[0]=res_str;__retbuf[1]=make_int(cnt_val);return value_copy(__retbuf[0]);}
static Value lua_string_format(int n,Value*a){if(n<1||a[0].t!=T_STRING)STR_FREE_RET(n,a,make_string(""));const char*fmt=luastr_cstr(a[0].ls);size_t bufsz=256;char*buf=malloc(bufsz);if(!buf)STR_FREE_RET(n,a,make_string(""));size_t buflen=0;int argi=1;while(*fmt){if(*fmt!='%'){BA(*fmt++);continue;}fmt++;if(*fmt=='%'){BA('%');fmt++;continue;}char spec[64];int si=0;spec[si++]='%';while(*fmt&&strchr("-+ #0",*fmt))spec[si++]=*fmt++;while(*fmt&&isdigit(*fmt))spec[si++]=*fmt++;if(*fmt=='.'){spec[si++]=*fmt++;while(*fmt&&isdigit(*fmt))spec[si++]=*fmt++;}char conv=*fmt++;switch(conv){case'd':case'i':case'u':case'o':case'x':case'X':spec[si++]='l';spec[si++]='l';spec[si++]=conv;spec[si]='\0';break;default:spec[si++]=conv;spec[si]='\0';}char tmp[256];Value av=argi<n?value_copy(a[argi]):make_nil();argi++;switch(conv){case'd':case'i':snprintf(tmp,sizeof tmp,spec,(long long)(av.t==T_INT?av.i:(int64_t)av.d));BAS(tmp,strlen(tmp));break;case'u':snprintf(tmp,sizeof tmp,spec,(unsigned long long)(av.t==T_INT?(uint64_t)av.i:(uint64_t)(int64_t)av.d));BAS(tmp,strlen(tmp));break;case'o':case'x':case'X':snprintf(tmp,sizeof tmp,spec,(unsigned long long)(av.t==T_INT?(uint64_t)av.i:(uint64_t)(int64_t)av.d));BAS(tmp,strlen(tmp));break;case'f':case'e':case'E':case'g':case'G':snprintf(tmp,sizeof tmp,spec,numval(av));BAS(tmp,strlen(tmp));break;case'c':BA((char)(av.t==T_INT?av.i:(int64_t)av.d));break;case's':{const char*sv;char sb[64];if(av.t==T_STRING)sv=luastr_cstr(av.ls);else if(av.t==T_INT){snprintf(sb,sizeof sb,"%lld",(long long)av.i);sv=sb;}else if(av.t==T_DOUBLE){snprintf(sb,sizeof sb,"%g",av.d);sv=sb;}else sv="nil";snprintf(tmp,sizeof tmp,spec,sv);BAS(tmp,strlen(tmp));break;}case'q':{BA('"');const char*sv;if(av.t==T_STRING)sv=luastr_cstr(av.ls);else{char qb[64];if(av.t==T_INT)snprintf(qb,sizeof qb,"%lld",(long long)av.i);else if(av.t==T_DOUBLE)snprintf(qb,sizeof qb,"%g",av.d);else snprintf(qb,sizeof qb,"nil");sv=qb;}while(*sv){if(*sv=='"'||*sv=='\\'||*sv=='\n'){BA('\\');}BA(*sv++);}BA('"');break;}default:BA(conv);break;}free_value(av);}if(buf)buf[buflen]='\0';for(int i=0;i<n;i++)free_value(a[i]);Value r=buf?make_string(buf):make_string("");free(buf);return r;}
#undef STR_FREE_RET
#undef BA
#undef BAS
static Value make_string_table(void){Value t=make_table();
#define SS(name) table_raw_set(t.tbl,make_string(#name),make_func(lua_string_##name))
SS(len);SS(sub);SS(rep);SS(reverse);SS(upper);SS(lower);SS(byte);SS(char);SS(find);SS(match);SS(gmatch);SS(gsub);SS(format);
#undef SS
return t;}
]==])
end
...
-- Globals (using renamed C vars for stdlib)
w("/* globals */")
for name in pairs(globals) do w("static Value "..name..";") end
wif(use.math_lib,   "static Value __lua_math;")
wif(use.os_lib,     "static Value __lua_os;")
wif(use.io_lib,     "static Value __lua_io;")
wif(use.string_lib, "static Value __lua_string;")
w("static Value __lua_arg;")
w("")
...
wif(use.math_lib,   "    __lua_math=make_math_table();")
wif(use.os_lib,     "    __lua_os=make_os_table();")
wif(use.io_lib,     "    __lua_io=make_io_table();")
wif(use.string_lib, "    __lua_string=make_string_table();")
w([[    __lua_arg=make_table();
    for(int __ai=1;__ai<argc;__ai++){
        table_raw_set(__lua_arg.tbl,make_int(__ai),make_string(argv[__ai]));
    }]])
...
while ti <= #topstmts do
    ...
    if av and method_registry[av] then
        for _,m in ipairs(method_registry[av]) do
            w(('    table_raw_set(%s.tbl,make_string("%s"),make_func(%s));'):format(global_cname(av),m.meth,m.fname)) end
...
for tbl_name,methods in pairs(method_registry) do
    for _,m in ipairs(methods) do
        w(('    table_raw_set(%s.tbl,make_string("%s"),make_func(%s));'):format(global_cname(tbl_name),m.meth,m.fname)) end
end

w("    /* cleanup */")
for name in pairs(globals) do w("    free_value("..name..");") end
wif(use.math_lib,   "    free_value(__lua_math);")
wif(use.os_lib,     "    free_value(__lua_os);")
wif(use.io_lib,     "    free_value(__lua_io);")
wif(use.string_lib, "    free_value(__lua_string);")
w("    free_value(__lua_arg);")
w("    return 0;\n}")
